...
for i in range(1, driverCount+1):
	#drivers.append(Visualizer(width=w, height=h, port=1610+i, pixelSize = 8, stayTop=True))
	drivers.append(Serial(ledtype, w*h, deviceID = i, SPISpeed = 2))
...
led = Matrix(driver = drivers, threadedUpdate = True, width = w, height = h*len(drivers), coordMap=build.map)
